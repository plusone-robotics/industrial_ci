...
* **ROSINSTALL_AUTOMERGE** (default: true): When set to true, any `wstool merge` operations will have the `-y` option set, preventing any CLI prompts from appearing when any element details are changed. Set this to false in order to have visibility on element detail changes. WARNING: If this is set to false, and a prompt occurs due to detail changes, the running CI process will halt indefinitely.
